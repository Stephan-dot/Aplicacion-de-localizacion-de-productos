...
// GRASP: Controller - gestiona el flujo de alta de producto y uso de datos del contexto.
export default function AddProduct() {
  const { 
    register, handleSubmit, formState: { errors, isSubmitting },watch,reset} = useForm({mode: 'onChange', 
      defaultValues: {
      name: '',
      type: 'medicamento',
      commerceId: '',
  ...
  const { addProduct, commerces, currentUser } = useApp();
  ...
  const onSubmit = async (data) => {
    ...
    console.log("FotoUrl: ", uploadImage)
    const productData = {
      producto: data.name,
      tipo: data.type,
      ubicacion: [commerce.ubicacion[0], commerce.ubicacion[1]],
      precio: parseFloat(data.price),
      esDisponible: data.available,
      comercioId: data.commerceId,
      fotoUrl: fotoUrl || randomImage,
    };
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      ...
      >
        ...
        <h1 className="text-3xl font-bold text-gray-800">Registrar Producto</h1>
        <p className="text-gray-600 mt-2">Agrega un nuevo producto disponible</p>
      </motion.div>
      ...
      {userCommerces.length === 0 ? (
        ...
        >
          ...
          <p className="text-gray-600 mb-6">
            Primero debes registrar un comercio antes de agregar productos
          </p>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate('/comercios/nuevo')}
            className="bg-gradient-to-r from-emerald-500 to-blue-500 text-white px-6 py-3 rounded-xl font-semibold"
          >
            Registrar Comercio
          </motion.button>
        </motion.div>
